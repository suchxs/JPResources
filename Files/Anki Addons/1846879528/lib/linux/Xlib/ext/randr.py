...
RandR_Rates = rq.Struct(rq.LengthOf("rates", 2), rq.List("rates", rq.Card16Obj))
...
def _1_0set_screen_config(
    self, size_id, rotation, config_timestamp, timestamp=X.CurrentTime
):
...
def set_screen_config(
    self, size_id, rotation, config_timestamp, rate=0, timestamp=X.CurrentTime
):
...
def set_screen_size(
    self, width, height, width_in_millimeters=None, height_in_millimeters=None
):
...
def get_output_property(
    self, output, property, type, long_offset, long_length, delete=False, pending=False
):
...
def set_crtc_config(
    self, crtc, config_timestamp, x, y, mode, rotation, outputs, timestamp=X.CurrentTime
):
...
def init(disp, info):
    ...
    disp.extension_add_method(
        "drawable", "xrandr_1_0set_screen_config", _1_0set_screen_config
    )
    disp.extension_add_method("drawable", "xrandr_set_screen_config", set_screen_config)
    disp.extension_add_method(
        "window", "xrandr_get_screen_size_range", get_screen_size_range
    )
    disp.extension_add_method("window", "xrandr_set_screen_size", set_screen_size)
    disp.extension_add_method(
        "window", "xrandr_get_screen_resources", get_screen_resources
    )
    disp.extension_add_method("display", "xrandr_get_output_info", get_output_info)
    disp.extension_add_method(
        "display", "xrandr_list_output_properties", list_output_properties
    )
    disp.extension_add_method(
        "display", "xrandr_query_output_property", query_output_property
    )
    disp.extension_add_method(
        "display", "xrandr_configure_output_property ", configure_output_property
    )
    disp.extension_add_method(
        "display", "xrandr_change_output_property", change_output_property
    )
    disp.extension_add_method(
        "display", "xrandr_delete_output_property", delete_output_property
    )
    disp.extension_add_method(
        "display", "xrandr_get_output_property", get_output_property
    )
    ...
    disp.extension_add_method(
        "display", "xrandr_delete_output_mode", delete_output_mode
    )
    ...
    disp.extension_add_method(
        "display", "xrandr_get_crtc_gamma_size", get_crtc_gamma_size
    )
    ...
    disp.extension_add_method(
        "window", "xrandr_get_screen_resources_current", get_screen_resources_current
    )
    disp.extension_add_method(
        "display", "xrandr_set_crtc_transform", set_crtc_transform
    )
    disp.extension_add_method(
        "display", "xrandr_get_crtc_transform", get_crtc_transform
    )
    ...
    if version.major_version == 1 and version.minor_version >= 5:
        ...
        disp.extension_add_event(
            info.first_event + RRScreenChangeNotify, ScreenChangeNotify
        )
        # add RRNotify events (1 event code with 3 subcodes)
        disp.extension_add_subevent(
            info.first_event + RRNotify, RRNotify_CrtcChange, CrtcChangeNotify
        )
        disp.extension_add_subevent(
            info.first_event + RRNotify, RRNotify_OutputChange, OutputChangeNotify
        )
        disp.extension_add_subevent(
            info.first_event + RRNotify, RRNotify_OutputProperty, OutputPropertyNotify
        )
