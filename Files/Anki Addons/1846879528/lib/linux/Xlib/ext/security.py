...
def generate_authorization(
    ...
    return SecurityGenerateAuthorization(
        display=self.display,
        opcode=self.display.get_extension_major(extname),
...
class SecurityRevokeAuthorization(rq.Request):
    _request = rq.Struct(
        rq.Card8("opcode"), rq.Opcode(2), rq.RequestLength(), AUTHID("authid")
    )


def revoke_authorization(self, authid):
    return SecurityRevokeAuthorization(
        display=self.display,
        opcode=self.display.get_extension_major(extname),
        authid=authid,
    )
...
def init(disp, info):
    disp.extension_add_method("display", "security_query_version", query_version)
    disp.extension_add_method(
        "display", "security_generate_authorization", generate_authorization
    )
    disp.extension_add_method(
        "display", "security_revoke_authorization", revoke_authorization
    )
