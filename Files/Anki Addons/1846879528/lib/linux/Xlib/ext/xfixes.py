...
class HideCursor(rq.Request):
    _request = rq.Struct(
        rq.Card8("opcode"), rq.Opcode(29), rq.RequestLength(), rq.Window("window")
    )
...
class ShowCursor(rq.Request):
    _request = rq.Struct(
        rq.Card8("opcode"), rq.Opcode(30), rq.RequestLength(), rq.Window("window")
    )
...
def init(disp, info):
    disp.extension_add_method(
        "display", "xfixes_select_selection_input", select_selection_input
    )
    ...
    disp.extension_add_subevent(
        info.first_event + XFixesCursorNotify,
