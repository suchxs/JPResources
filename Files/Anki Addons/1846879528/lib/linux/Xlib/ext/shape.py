...
class KIND(rq.Set):
    def __init__(self, name):
        super(KIND, self).__init__(name, 1, values=(SK.Bounding, SK.Clip, SK.Input))
...
def rectangles(
    self, operation, destination_kind, ordering, x_offset, y_offset, rectangles
):
    Rectangles(
        display=self.display,
        opcode=self.display.get_extension_major(extname),
...
def init(disp, info):
    ...
    disp.extension_add_event(
        info.first_event + Event.Notify, NotifyEventData, "ShapeNotify"
    )
