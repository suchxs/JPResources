...
class DamageDestroy(rq.Request):
    _request = rq.Struct(
        rq.Card8("opcode"), rq.Opcode(2), rq.RequestLength(), DAMAGE("damage")
    )
...
def init(disp, info):
    disp.extension_add_method("display", "damage_query_version", query_version)

    disp.extension_add_method("drawable", "damage_create", damage_create)

    disp.extension_add_method("display", "damage_destroy", damage_destroy)

    disp.extension_add_method("display", "damage_subtract", damage_subtract)

    disp.extension_add_method("drawable", "damage_add", damage_add)
