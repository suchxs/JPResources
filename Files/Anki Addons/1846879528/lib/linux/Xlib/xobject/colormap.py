...
rgb_res = [
    ...
    re.compile(
        r"\A#([0-9a-fA-F][0-9a-fA-F])([0-9a-fA-F][0-9a-fA-F])([0-9a-fA-F][0-9a-fA-F])\Z"
    ),
    re.compile(
...
class Colormap(resource.Resource):
    ...
    def free(self, onerror=None):
        request.FreeColormap(display=self.display, onerror=onerror, cmap=self.id)
    ...
    def alloc_color(self, red, green, blue):
        return request.AllocColor(
            display=self.display, cmap=self.id, red=red, green=green, blue=blue
        )
    ...
    def alloc_named_color(self, name):
        ...
        try:
            return request.AllocNamedColor(
                display=self.display, cmap=self.id, name=name
            )
        except error.BadName:
            return None

    def alloc_color_cells(self, contiguous, colors, planes):
        return request.AllocColorCells(
    ...
    def alloc_color_planes(self, contiguous, colors, red, green, blue):
        return request.AllocColorPlanes(
            display=self.display,
            contiguous=contiguous,
            cmap=self.id,
            colors=colors,
    ...
    def store_colors(self, items, onerror=None):
        request.StoreColors(
            display=self.display, onerror=onerror, cmap=self.id, items=items
        )
