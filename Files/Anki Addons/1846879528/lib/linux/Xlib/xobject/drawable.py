...
class Drawable(resource.Resource):
    ...
    def create_gc(self, **keys):
        ...
        request.CreateGC(display=self.display, cid=cid, drawable=self.id, attrs=keys)

        cls = self.display.get_resource_class("gc", fontable.GC)
        return cls(self.display, cid, owner=1)

    def copy_area(
        self, gc, src_drawable, src_x, src_y, width, height, dst_x, dst_y, onerror=None
    ):
        request.CopyArea(
            display=self.display,
            onerror=onerror,
    ...
    def poly_arc(self, gc, arcs, onerror=None):
        request.PolyArc(
            display=self.display, onerror=onerror, drawable=self.id, gc=gc, arcs=arcs
        )
    ...
    def put_image(
        self, gc, x, y, width, height, format, depth, left_pad, data, onerror=None
    ):
        request.PutImage(
            display=self.display,
            onerror=onerror,
    ...
    def put_pil_image(self, gc, x, y, image, onerror=None):
        ...
        maxlen = (
            self.display.info.max_request_length << 2
...
class Window(Drawable):
    ...
    def change_attributes(self, onerror=None, **keys):
        request.ChangeWindowAttributes(
            display=self.display, onerror=onerror, window=self.id, attrs=keys
        )

    def get_attributes(self):
        return request.GetWindowAttributes(display=self.display, window=self.id)

    def destroy(self, onerror=None):
        request.DestroyWindow(display=self.display, onerror=onerror, window=self.id)
    ...
    def change_save_set(self, mode, onerror=None):
        request.ChangeSaveSet(
            display=self.display, onerror=onerror, mode=mode, window=self.id
        )
    ...
    def map(self, onerror=None):
        request.MapWindow(display=self.display, onerror=onerror, window=self.id)

    def map_sub_windows(self, onerror=None):
        request.MapSubwindows(display=self.display, onerror=onerror, window=self.id)

    def unmap(self, onerror=None):
        request.UnmapWindow(display=self.display, onerror=onerror, window=self.id)
    ...
    def circulate(self, direction, onerror=None):
        request.CirculateWindow(
            display=self.display, onerror=onerror, direction=direction, window=self.id
        )
    ...
    def change_text_property(
        self, property, property_type, data, mode=X.PropModeReplace, onerror=None
    ):
        ...
        self.change_property(
            property, property_type, 8, data, mode=mode, onerror=onerror
        )

    def delete_property(self, property, onerror=None):
        request.DeleteProperty(
            display=self.display, onerror=onerror, window=self.id, property=property
        )

    def get_property(self, property, property_type, offset, length, delete=False):
        r = request.GetProperty(
            display=self.display,
            delete=delete,
            window=self.id,
            property=property,
        ...
        if prop:
            ...
            if prop.bytes_after:
                prop = self.get_property(
                    property, property_type, sizehint, prop.bytes_after // 4 + 1
                )
                val = val + prop.value
    ...
    def get_full_text_property(
        self, property, property_type=X.AnyPropertyType, sizehint=10
    ):
        prop = self.get_full_property(property, property_type, sizehint=sizehint)
    ...
    def list_properties(self):
        r = request.ListProperties(display=self.display, window=self.id)
        return r.atoms
    ...
    def grab_key(
        self, key, modifiers, owner_events, pointer_mode, keyboard_mode, onerror=None
    ):
        request.GrabKey(
            display=self.display,
            onerror=onerror,
    ...
    def get_motion_events(self, start, stop):
        r = request.GetMotionEvents(
            display=self.display, window=self.id, start=start, stop=stop
        )
        return r.events

    def translate_coords(self, src_window, src_x, src_y):
        return request.TranslateCoords(
            display=self.display,
            src_wid=src_window,
            dst_wid=self.id,
            src_x=src_x,
    ...
    def create_colormap(self, visual, alloc):
        ...
        request.CreateColormap(
            display=self.display, alloc=alloc, mid=mid, window=self.id, visual=visual
        )
    ...
    def set_wm_name(self, name, onerror=None):
        self.change_text_property(Xatom.WM_NAME, Xatom.STRING, name, onerror=onerror)
    ...
    def set_wm_icon_name(self, name, onerror=None):
        self.change_text_property(
            Xatom.WM_ICON_NAME, Xatom.STRING, name, onerror=onerror
        )
    ...
    def set_wm_class(self, inst, cls, onerror=None):
        self.change_text_property(
            Xatom.WM_CLASS, Xatom.STRING, "%s\0%s\0" % (inst, cls), onerror=onerror
        )
    ...
    def set_wm_transient_for(self, window, onerror=None):
        self.change_property(
            Xatom.WM_TRANSIENT_FOR, Xatom.WINDOW, 32, [window.id], onerror=onerror
        )
    ...
    def set_wm_protocols(self, protocols, onerror=None):
        self.change_property(
            self.display.get_atom("WM_PROTOCOLS"),
            Xatom.ATOM,
            32,
            protocols,
            onerror=onerror,
    ...
    def set_wm_colormap_windows(self, windows, onerror=None):
        self.change_property(
            self.display.get_atom("WM_COLORMAP_WINDOWS"),
            Xatom.WINDOW,
            32,
    ...
    def get_wm_colormap_windows(self):
        d = self.get_full_property(
            self.display.get_atom("WM_COLORMAP_WINDOWS"), Xatom.WINDOW
        )
        if d is None or d.format != 32:
            return []
        else:
            cls = self.display.get_resource_class("window", Window)
            return map(lambda i, d=self.display, c=cls: c(d, i), d.value)

    def set_wm_client_machine(self, name, onerror=None):
        self.change_text_property(
            Xatom.WM_CLIENT_MACHINE, Xatom.STRING, name, onerror=onerror
        )
    ...
    def get_wm_normal_hints(self):
        return self._get_struct_prop(
            Xatom.WM_NORMAL_HINTS, Xatom.WM_SIZE_HINTS, icccm.WMNormalHints
        )

    def set_wm_hints(self, hints={}, onerror=None, **keys):
        self._set_struct_prop(
            Xatom.WM_HINTS, Xatom.WM_HINTS, icccm.WMHints, hints, keys, onerror
        )
    ...
    def get_wm_icon_size(self):
        return self._get_struct_prop(
            Xatom.WM_ICON_SIZE, Xatom.WM_ICON_SIZE, icccm.WMIconSize
        )
...
class Pixmap(Drawable):
    ...
    def free(self, onerror=None):
        request.FreePixmap(display=self.display, onerror=onerror, pixmap=self.id)
