...
class Fontable(resource.Resource):
    ...
    def query_text_extents(self, string):
        return request.QueryTextExtents(
            display=self.display, font=self.id, string=string
        )
...
class GC(Fontable):
    ...
    def set_clip_rectangles(
        self, x_origin, y_origin, rectangles, ordering, onerror=None
    ):
...
class Font(Fontable):
    ...
    def close(self, onerror=None):
        request.CloseFont(display=self.display, onerror=onerror, font=self.id)
        self.display.free_resource_id(self.id)
